# Place des Montres × Plateforme e-commerce horlogère

**Fiche récapitulative** — à remettre au client après la présentation  
*Document confidentiel · Juin 2026*

---

## Votre situation

**Place des Montres** — spécialiste horloger à Strasbourg depuis 1995  
3 000 montres en stock · 30 marques · boutique Place des Halles · vente + réparation

**Site actuel :** PrestaShop — maintenance, hébergement et évolutions à votre charge.

---

## Notre proposition

Une **solution e-commerce sur-mesure, dédiée à votre maison**, avec **hébergement et maintenance managés**. Votre futur site est **déjà dessiné pour vous** : identité visuelle, textes, FAQ, services, règles Colissimo (gratuit dès 80 €) et retrait magasin.

**Fiabilité :** socle technique déjà éprouvé en conditions réelles de production e-commerce — commandes, paiements et administration opérationnels.

---

## Ce que vous gagnez

| Aujourd'hui (PrestaShop) | Demain (notre plateforme) |
|---|---|
| Maintenance technique lourde | Hébergement cloud managé (Vercel, Render, Supabase) |
| Back-office générique | Admin horloger : montres, commandes, promos, carrousel |
| UX mobile datée | Parcours moderne, mega-menu marques, checkout Stripe |
| Évolutions coûteuses | Évolutions continues incluses |
| Risque SEO à la migration | Import catalogue + redirections 301 automatisées |

---

## Fonctionnalités clés (déjà en place pour vous)

- Catalogue retail 3 000+ références, filtres marque / genre / promo
- Checkout : Colissimo + retrait Place des Halles, codes promo, réservation stock 30 min
- Pages : À propos, Services, Guide de l'horloger, FAQ (15 questions), Contact + carte
- Admin : dashboard CA, gestion stock, commandes, promos, carrousel accueil, statistiques
- Conformité : RGPD, cookies, Stripe PCI, pages légales

---

## Tarifs indicatifs

| Prestation | Montant |
|---|---|
| **Migration PrestaShop** (one-shot) | **4 900 € HT** |
| ↳ Import ~3 000 références, bascule DNS, SEO 301, formation 2 h | |
| **Hébergement + maintenance** | **à partir de 249 € HT / mois** |
| ↳ Infra, mises à jour, monitoring, 2 h support/mois (réponse sous 4 h ouvrées) | |
| **Engagement recommandé** | 12 mois |

*Devis définitif après audit export PrestaShop. Domaine et données restent votre propriété.*

---

## Prochaines étapes

1. **Démo guidée** — 45 min sur votre site, déjà à vos couleurs
2. **Audit express** — export catalogue PrestaShop
3. **Devis sous 48 h**
4. **Bascule production** — fenêtre à définir ensemble

---

## Contact

**Doryan Dillen**  
doryandillen@gmail.com

> *« Vous avez passé 30 ans à construire la confiance de vos clients à Strasbourg. Nous portons cette même exigence en ligne — sans que la technique ne vous en distraie un jour de plus. »*
