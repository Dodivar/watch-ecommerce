...
/**
 * Montre exposée dans le panneau du hero « vitrine », à partir d'une sélection ordonnée.
 *
 * Seule la tête de liste est montrée : les suivantes sont des remplaçantes, prises dans
 * l'ordre dès que celle du dessus part à la vente. Sélection vide ou entièrement vendue,
 * on retombe sur le comportement d'origine — la dernière montre disponible du catalogue.
 *
 * Deux appelants : la résolution publique ci-dessous, et l'aperçu de l'écran
 * d'administration, qui doit exposer le brouillon avec exactement les mêmes données que la
 * page d'accueil — l'image de la montre en premier lieu.
 *
 * @param {Array<{ id: string, is_sold?: boolean }> | null | undefined} candidates
 * @param {{ loadWatch?: (id: string) => Promise<object | null> }} [options]
 *   `loadWatch` permet à l'aperçu admin de mémoriser les montres déjà chargées.
 * @returns {Promise<Record<string, unknown> | null>}
 */
export async function assembleVitrineWatch(candidates, options = {}) {
  const { loadWatch = getWatchById } = options

  for (const candidate of candidates ?? []) {
    if (!candidate?.id || !isShowcasable(candidate)) continue
    // Repasser par `getWatchById` : la ligne jointe n'a pas ses images ni ses relations.
    const watch = await Promise.resolve(loadWatch(candidate.id)).catch(() => null)
...
/**
 * Montre exposée dans le panneau du hero « vitrine » : la sélection admin
 * (`home_featured_watches`, contexte `vitrine`), assemblée comme ci-dessus.
 *
 * @returns {Promise<Record<string, unknown> | null>}
 */
export async function resolveVitrineWatch() {
  ...
  return assembleVitrineWatch(featured)
}
